...
function uri(text) {
	return window.URI.withinString(text, function(url) {
		...
		var split = url.split("<");
		url = "<a href='" + split[0].replace(/^www/, "http://www") + "' target='_blank' rel='noopener noreferrer'>" + split[0] + "</a>";
		if (split.length > 1) {
...
export class LinkerizedText extends React.Component {
	render() {
		return <span dangerouslySetInnerHTML={{__html: parse(this.props.text)}} />;
	}
}

export const UserName = ({mode, nick}) =>
	<span role="button" className={classNames("user", colorClass(nick))} data-name={nick}>{mode}{nick}</span>;
